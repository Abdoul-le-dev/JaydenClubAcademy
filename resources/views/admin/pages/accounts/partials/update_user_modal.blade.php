        ...
        <button type="button" class="btn btn-primary waves-effect waves-light"  data-toggle="modal" data-target="#updateUserModal_{{$user->id}}">
            Modifier
         </button>

         <!-- Modal -->
         <form  method="post" action="{{route('dashboard.accounts.update',['id'=>$user->id])}}">
             @csrf
         <div class="modal fade" id="updateUserModal_{{$user->id}}" tabindex="-1" role="dialog" aria-labelledby="createEquipeModalTitle" aria-hidden="true">
